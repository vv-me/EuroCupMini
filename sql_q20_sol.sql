...
SELECT COUNT(player_id) FROM player_booked WHERE play_half = 1 and play_schedule = 'NT';
-- If you are asking for total count of substitions in per match
SELECT COUNT(player_id), match_no FROM player_booked WHERE play_half = 1 and play_schedule = 'NT'
GROUP BY match_no;
-- If you are asking for total count of players who were substituted 
SELECT COUNT(DISTINCT player_id) FROM player_booked WHERE play_half = 1 and play_schedule = 'NT'
